...
Date.jab2date = function(ts) {
	...
	if (ts.substr(ts.length - 6, 1) != 'Z') {
		...
		if (ts.substr(ts.length - 6, 1) == '+') date.setTime(date.getTime() - offset.getTime());
		else if (ts.substr(ts.length - 6, 1) == '-') date.setTime(date.getTime() + offset.getTime());
	}
	return date;
};
...
function utf8t2d(t) {
	...
	if (test.charCodeAt(0) < 0) for (var n = 0; n < t.length; n++) {
...
var JSJaCBuilder = {
	buildNode: function(doc, elementName) {
		...
		} else element = this._createElement(doc, elementName, ns);
	...
	_attributes: function(attributes) {
		var attrs = [];
		for (attribute in attributes) if (attributes.hasOwnProperty(attribute)) attrs.push(attribute + '="' + attributes[attribute].toString().htmlEnc() + '"');
		return attrs.join(" ");
	},
...
JSJaCPacket.prototype.getChildVal = function(name, ns) {
	...
	if (node && node.hasChildNodes()) {
		for (var i = 0; i < node.childNodes.length; i++) if (node.childNodes.item(i).nodeValue) ret += node.childNodes.item(i).nodeValue;
	}
	return ret;
};
...
JSJaCPacket.prototype._replaceNode = function(aNode) {
	for (var i = 0; i < aNode.attributes.length; i++) if (aNode.attributes.item(i).nodeName != 'xmlns') this.getNode().setAttribute(aNode.attributes.item(i).nodeName, aNode.attributes.item(i).nodeValue);
	for (var i = 0; i < aNode.childNodes.length; i++) if (this.getDoc().importNode) this.getNode().appendChild(this.getDoc().importNode(aNode.childNodes.item(i), true));
...
JSJaCIQ.prototype.reply = function(payload) {
	...
	if (payload) {
		if (typeof payload == 'string') rIQ.getChild().appendChild(rIQ.getDoc().loadXML(payload));
			...
			for (var i = 0; i < payload.length; i++) if (typeof payload[i] == 'string') node.appendChild(rIQ.getDoc().loadXML(payload[i]));
...
function JSJaCConnection(oArg) {
	...
	else this.allow_plain = JSJAC_ALLOW_PLAIN;
...
JSJaCConnection.prototype.unregisterHandler = function(event, handler) {
	...
	res = new Array();
	for (var i = 0; i < arr.length; i++) if (arr[i].handler != handler) res.push(arr[i]);
...
JSJaCConnection.prototype.resume = function() {
	try {
		...
		if (this._keys) {
			...
			for (var i = 0; i < u.length; i++) this._keys2[u[i]] = this._keys[u[i]];
...
JSJaCConnection.prototype.send = function(packet, cb, arg) {
	...
	if (cb) {
		if (!packet.getID()) packet.setID('JSJaCID_' + this._ID++);
...
JSJaCConnection.prototype.suspend = function() {
	...
	for (var i = 0; i < u.length; i++) {
		...
		if (this[u[i]]._getSuspendVars) {
			...
			var o = new Object();
			for (var j = 0; j < uo.length; j++) o[uo[j]] = this[u[i]][uo[j]];
		} else var o = this[u[i]];
		s[u[i]] = o;
	}
...
JSJaCConnection.prototype._parseStreamFeatures = function(doc) {
	...
	var errorTag;
	if (doc.getElementsByTagNameNS) errorTag = doc.getElementsByTagNameNS("http://etherx.jabber.org/streams", "error").item(0);
	else {
		var errors = doc.getElementsByTagName("error");
		for (var i = 0; i < errors.length; i++) if (errors.item(i).namespaceURI == "http://etherx.jabber.org/streams") {
	...
	this.has_sasl = false;
	for (var i = 0; i < lMec1.length; i++) if (lMec1.item(i).getAttribute("xmlns") == "urn:ietf:params:xml:ns:xmpp-sasl") {
		this.has_sasl = true;
		var lMec2 = lMec1.item(i).getElementsByTagName("mechanism");
		for (var j = 0; j < lMec2.length; j++) this.mechs[lMec2.item(j).firstChild.nodeValue] = true;
		break;
	}
	this.has_sasl = false;
...
JSJaCHttpBindingConnection.prototype._getRequestString = function(raw, last) {
	...
	if (this._rid <= this._last_rid && typeof(this._last_requests[this._rid]) != 'undefined') reqstr = this._last_requests[this._rid].xml;
		...
		if (last) reqstr += "type='terminate'";
			...
			if (JSJACHBC_USE_BOSH_VER) reqstr += "xmpp:restart='true' xmlns:xmpp='urn:xmpp:xbosh'";
			this._reinit = false;
		}
		if (xml != '' || raw != '') {
			reqstr += ">" + raw + xml + "</body>";
		...
		for (var i in this._last_requests) if (this._last_requests.hasOwnProperty(i) && i < this._rid - this._hold) delete(this._last_requests[i]);
	}
	return reqstr;
};
JSJaCHttpBindingConnection.prototype._getInitialRequestString = function() {
	...
	if (this._xmllang) reqstr += " xml:lang='" + this._xmllang + "'";
...
JSJaCHttpBindingConnection.prototype._handleInitialResponse = function(slot) {
	...
	if (body.getAttribute('maxpause')) this._pause = Number.max(body.getAttribute('maxpause'), JSJACHBC_MAXPAUSE);
	this.setPollInterval(this._timerval);
	this._connected = true;
	this._inQto = setInterval(JSJaC.bind(this._checkInQ, this), JSJAC_CHECKINQUEUEINTERVAL);
	this._interval = setInterval(JSJaC.bind(this._checkQueue, this), JSJAC_CHECKQUEUEINTERVAL);
...
JSJaCHttpBindingConnection.prototype._parseResponse = function(req) {
	...
	if (body.getAttribute("type") == "terminate") {
		...
		clearInterval(this._interval);
		clearInterval(this._inQto);
		if (body.getAttribute("condition") == "remote-stream-error") if (body.getElementsByTagName("conflict").length > 0) this._setStatus("session-terminate-conflict");
...
JSJaCHttpPollingConnection.prototype._getInitialRequestString = function() {
	...
	if (this.authhost) streamto = this.authhost;
	reqstr += ",<stream:stream to='" + streamto + "' xmlns='jabber:client' xmlns:stream='http://etherx.jabber.org/streams'";
	if (this.authtype == 'sasl' || this.authtype == 'saslanon') reqstr += " version='1.0'";
	reqstr += ">";
	return reqstr;
};
...
JSJaCHttpPollingConnection.prototype._getStreamID = function() {
	...
	try {
		...
		if (!response.match(/<\/stream:stream>\s*$/)) response += '</stream:stream>';
...
JSJaCHttpPollingConnection.prototype._parseResponse = function(r) {
	...
	if (!req.responseText || req.responseText == '') return null;
	try {
		var response = req.responseText.replace(/\<\?xml.+\?\>/, "");
		if (response.match(/<stream:stream/)) response += "</stream:stream>";
		...
		if (!doc || doc.tagName == 'parsererror') {
			...
			if (doc && doc.tagName != 'parsererror') {
				...
				if (doc.getElementsByTagName('conflict').length > 0) this._setStatus("session-terminate-conflict");
				clearTimeout(this._timeout);
				clearInterval(this._interval);
				clearInterval(this._inQto);
				this._handleEvent('onerror', JSJaCError('503', 'cancel', 'session-terminate'));
				...
				this.oDbg.log("Disconnected.", 1);
				this._handleEvent('ondisconnect');
			} else this.oDbg.log("parsererror:" + doc, 1);
			return doc;
		}
		return doc;
	} catch(e) {
...
var JSJaC = {
	...
	load: function() {
		...
		for (var i = 0; i < includes.length; i++) this.require(path + includes[i] + '.js');
	},
