...
var remote = {
    ...
    jsjac: {
        httpbase: window.contextPath + "/JHB/", 
        domain: window["serverDomin"],
        username: "",
        pass: "",
        timerval: 3000, // 设置请求超时
...
remote.jsjac.chat = {
    ...
    //mrtom send iq message
    sendIQMessage: function (msg, to) {
    	 ...
    	 try {
             ...
             var iq = new JSJaCIQ();
             iq.setType("get");
             iq.setTo(remote.jsjac.domain);
             iq.setFrom(to.split('/')[0]);
             var node = iq.setQuery('jabber:iq:extend:everySale');
             
             node.appendChild(iq.buildNode('action',"get_members"));
             node.appendChild(iq.buildNode('param',"1"));
             //iq.setXMLLang("get_members");
             iq.setID(userJID);           
                         
             alert('a------------'+iq.xml());
             
             // 发送消息
             remote.connection.send(iq);
    ...
    handleIQ: function (aIQ) {
        ...
        var html = "<div class='msg'>IN2 (raw): " + aIQ.xml().htmlEnc() + "</div>";
        var err = aIQ.errorReply(ERR_FEATURE_NOT_IMPLEMENTED);
        remote.console.info(html + "<Br/>"+err.xml());
        remote.connection.send(err);
    },
    //1.远程消息监听 mrtom
